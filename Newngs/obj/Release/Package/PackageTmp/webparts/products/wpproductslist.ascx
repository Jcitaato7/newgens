<%@ Control Language="vb" AutoEventWireup="false" CodeBehind="wpproductslist.ascx.vb" Inherits="tfsCMS.wpproductslist" %>
...
<script type="text/javascript">
    ...
    //        alert("");
    //        $(".productUrlHashWrap input[type=hidden]").val(window.location.hash);
...
<asp:Repeater ID="rptMainItem" runat="server">
    <ItemTemplate>        
        <div class="box">
            ...
            <div class="accordion"> 
                <asp:Repeater runat="server" ID="rptSubItems" DataSource='<%# Eval("Products")%>'>
                    <ItemTemplate>                    
                        <div id='<%#_clscode.Slugify(Eval("ProductName"))%>'> <a href='<%# ResolveUrl("~/product/" & Eval("ProductID") & "/" & _clscode.Slugify(Eval("ProductName")) & ".aspx")%>' class="tab"><%# Eval("ProductName")%></a>
<%--                            <div class="content">     
                                <p>
